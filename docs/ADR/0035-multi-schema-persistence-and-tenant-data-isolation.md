...
**Connection-level `search_path` safety.** Tenant scoping is set per operation, never left resident on a pooled connection: either `SET LOCAL search_path` inside a per-operation transaction, or a pool acquire/reset hook that restores the default `search_path` on release. A reused pooled connection must never carry a prior tenant's `search_path` (no cross-tenant read/write leakage); a no-leak integration test asserts this in CI.
