...
- Generated code under `packages/contracts/codegen/python/` is committed build output but **not yet a Python package** (no `__init__.py`). Consumers (`ingest-python`, `api-python`) must still decide how to import it — path injection, a local dependency, or a re-export shim. Note the Kafka payload models are two flat modules `machine_telemetry.py` / `line_state.py`; there is **no aggregate `kafka_payloads` module** (the plan's single-module import is obsolete — directory input forbids single-file codegen output).
