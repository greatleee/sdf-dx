...
- **ISO 22400-2 clause number: ADR-0012 says §5, everything else says §6.** `ADR-0012` cites ISO 22400-2:2014 **§5** (4×) for the OEE / A·P·Q definitions, but `GLOSSARY` (`monitoring` → *OEE* / *A·P·Q*), `DOMAIN-NOTES` (§"ISO 22400 — KPI definitions"), and the `monitoring/domain/oee.py` implementation all cite **§6** — and `DOMAIN-NOTES` records the research conclusion that clause 6 is "Description of KPIs" while clause 5 is the input *elements*. ADR-0012 additionally states its formulas are kept "identical to the GLOSSARY," which cites §6, so the ADR is internally inconsistent. **§6 is the likely-correct citation**; ADR-0012 left unedited here per the supersede-only doc policy — reconcile when ADR-0012 is next revised or superseded. (Does not affect the code: `oee.py` and its tests already cite §6.)
