...
| **Related** | ADR-0028 (FC/IS + generated-Zod boundary), ADR-0029 (live WebSocket → Query cache), ADR-0030 (URL-as-SoT + state split; router/store deferred), ADR-0031 (lint enforcement set + LLM-drift guardrails). Parents: ADR-0004 / 0005 / 0016 / 0018 / 0007 / 0022. |
...
Rules that aren't enforced get broken, especially under LLM-driven change. The backend learned this and elevated its enforcement set to an ADR of its own (ADR-0023: `import-linter` contracts + custom AST call-site checks); the frontend now does the same in **ADR-0031**, which is the authoritative list. The shape is three depths, cheapest first:

1. **Agent edit-time** — a Claude Code `PostToolUse` hook (`.claude/settings.json`) runs Prettier then ESLint on any `apps/dashboard-react/**/*.{ts,tsx}` an agent touches, so drift is reported in the same session it's introduced. This is where LLM drift originates, so it's where the first guard sits; it complements, never replaces, the layers below (a human hand-edit is caught by them, not the hook).
2. **CI lint** — `eslint-plugin-boundaries` fails the build on a wrong-direction import (the matrix now spells out the full `ui → application → ports → domain` direction, including `ui ↛ adapters` and `shared` held to domain-grade purity). On top of the boundary topology sit the FE analogs of the backend's AST checks: inside `domain/` and `shared/`, `no-restricted-syntax` / `no-restricted-globals` ban the clock/uuid/random call-sites (`Date.now()`, `Math.random()`, `crypto.randomUUID()`) and browser-IO globals (`fetch`, `window`, `localStorage`) that injection exists to avoid, and `no-restricted-imports` keeps `zod` and React out of the core. `switch-exhaustiveness-check` makes a missed union case a build error, `import/no-cycle` catches what the boundary matrix can't, and `complexity` caps cyclomatic complexity at 10 — the same number the backend uses, so a reviewer reads one threshold across the stack. The `@typescript-eslint` strict-type-checked set (no `any`, no floating promises, consistent type imports) and the contract drift gate (`make all` + `git diff --exit-code codegen/`) round it out.
3. **Formatting** — Prettier owns it (`eslint-config-prettier` switches off ESLint's stylistic rules so the two never fight; we don't run Prettier *as* a lint rule), giving the FE the formatter Python (`ruff format`) and Kotlin (`ktlint`) already had.

What lint *can't* catch is stated honestly in ADR-0031: a `new WebSocket` inside a component (§4/§5), business logic written inline in a JSX body (§1), and adapter role-prefix naming (§8) are left to code review and test placement, because a custom plugin would cost more than the drift it prevents. The documentary layer — `.claude/rules/frontend-code-architecture.md` (auto-loaded) for the operational do/don't, this doc for the why — backs all three. When any check fails, the fix is to restructure to the correct direction, never to add an ignore entry or smuggle a runtime import behind `import type`.
