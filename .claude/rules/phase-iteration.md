...
**Project-lifetime files** (already created; never re-write):
...
- `docs/spec/use-cases/_TEMPLATE.md` — leave untouched unless the UC structure itself changes
...
Distinguish four task kinds when evaluating order:

- **Create** — a task that *first writes* a spec/ADR/UC file (or initializes a living doc). Must be in Chapter 0.
- **Implementation** — a task that writes code, infrastructure, scaffold, or tests (`feat`, `chore`, `infra`, `test`, `fix`). Belongs at its natural phase position, *after* Chapter 0.
...
1. Every **Create** task must appear in Chapter 0, before any **Implementation** task.
2. No **Implementation** task may appear before Chapter 0 is complete.
3. **Promote / status-change** tasks at phase end are correct — do not flag.
4. A task that mixes multiple kinds (e.g., "write UC-002 spec [Create] + add E2E tests [Implementation] + flip status to implemented [Promote]") must be **split per kind**: Create → Chapter 0, Implementation → natural phase position, Promote → phase end. Each part lands as its own commit.
